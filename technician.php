...
<div class="col-sm-9 col-md-10 mt-5 text-center">
    <h4 class="bg-dark text-white">List Of Technicion</h4>
<?php
...
echo '<tbody>';
  ...
  echo '<input type="hidden" name="id" id="id" value='.$row['empid'].'><button type ="submit" class="btn btn-info mr-3" name="edit" value="Edit" ><i class="fas fa-pen"></i></button>';
  echo '</form>'; 
  echo '<form action="" method="POST" class="d-inline">';
  echo '<input type="hidden" name="id"  value='.$row['empid'].'><button type ="submit" class="btn btn-danger mr-3" name="delete" value="delete" ><i class="far fa-trash-alt"></i></button>';
  echo '</form>'; 
 echo '</td>';
  echo '</tr>';
  }
...
<div class="float-right">
    <a href="interemp.php" class="btn btn-danger"><i class="fas fa-plus fa-2x" ></i></a>
</div>
</div>
...
if (isset($_REQUEST['delete'])) {
    $sql = "DELETE FROM technition_tb WHERE empid = {$_REQUEST['id']}";
    if ($conn->query($sql)) {
